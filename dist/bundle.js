"use strict";
(() => {
  // src/vendor/pretext/src/bidi.ts
  var baseTypes = [
    "BN",
    "BN",
    "BN",
  ...
  // src/vendor/pretext/src/analysis.ts
  ...
  // src/vendor/pretext/src/measurement.ts
  ...
  // src/vendor/pretext/src/line-break.ts
  ...
  // src/vendor/pretext/src/layout.ts
  ...
  // src/main.ts
  var BIRTH = /* @__PURE__ */ new Date("1999-06-04T00:00:00");
  var UNITS = ["minutes", "hours", "days", "months", "years"];
  function getElapsed(unit) {
    ...
    switch (unit) {
      ...
      case "years":
        return ms / (864e5 * 365.25);
    }
  }
  function formatElapsed(unit) {
    const v = getElapsed(unit);
    return unit === "years" ? v.toFixed(9) : Math.floor(v).toString();
  }
  var pretextCache = /* @__PURE__ */ new Map();
  function measuredCharWidth(ch, fontSize, ctx) {
    const key = `${ch}_${fontSize}`;
    if (pretextCache.has(key))
      return pretextCache.get(key);
    let w = fontSize * 0.62;
    try {
      const prepared = prepareWithSegments(ch, `bold ${fontSize}px Quantico`);
      const result = layoutWithLines(prepared, 4e3, fontSize * 1.5);
      if (result.lines.length > 0)
        w = result.lines[0].width;
    } catch {
  ...
  function makeDigit(char, tx, ty, fontSize, delay) {
    return {
      char,
      tx,
      ty,
  ...
  function layoutDigits(str, fontSize, paddingLeft, paddingTop, maxWidth, ctx) {
  ...
  function buildDOM() {
    ...
    const unitButtons = /* @__PURE__ */ new Map();
  ...
  function setActiveButton(unitButtons, active) {
  ...
  function main() {
    ...
    function rebuildDigits(instant) {
